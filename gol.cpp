...
typedef uchar state_chart_t[2][2][2][2][2][2][2][2][2];
typedef uchar world_t[Height][Width];

void init_scene(GLuint&);
void destroy_scene(GLuint&);
void draw_scene(GLuint&);
template <typename T>
void update_texture(GLuint&, T);
...
namespace detail {
    ...
    {
    public:
        size_t operator()(const world_t& world) const
        {
...
struct timer
{
    ...
    void reset(string header)
    {
        ...
        cout << header << ' ' << chrono::duration_cast<chrono::milliseconds>(now - start).count() << " ms\n";
        start = now;
    }
};
...
void create_state_chart(state_chart_t chart)
{
    ...
    for(i.val = 0; i.val < (1<<9); i.val++)
    {
        auto sum = [&]() -> int {
            auto j = i.val - ((i.val>>1)&0x55555555);
            j = (j & 0x33333333) + ((j >> 2) & 0x33333333);
            return (((j + (j >> 4)) & 0x0F0F0F0F) * 0x01010101) >> 24;
        }() - i.b1.b8;
        //fmt.Printf("%v -%v-> %v\n", i, (i>>8)&1, sum)
        if(i.b1.b8 == 1)
            chart[i.b1.b8][i.b1.b7][i.b1.b6][i.b1.b5][i.b1.b4][i.b1.b3][i.b1.b2][i.b1.b1][i.b1.b0] =
                (sum == 2 || sum == 3) ?  255 : 0;
        else
            chart[i.b1.b8][i.b1.b7][i.b1.b6][i.b1.b5][i.b1.b4][i.b1.b3][i.b1.b2][i.b1.b1][i.b1.b0] =
                sum == 3 ? 255 : 0;
...
void update_world(world_t cur, world_t prev, state_chart_t chart)
{
    ...
    for(auto y = 0; y < Height; y++)
        for(auto x = 0; x < Width; x++)
        {
            const auto hy = Height+y;
            const auto wx = Width+x;
            cur[y][x] = chart[prev[y][x]/255][prev[(hy-1)%Height][(wx-1)%Width]/255][prev[(hy-1)%Height][x]/255][prev[(hy-1)%Height][(wx+1)%Width]/255][prev[y][(wx-1)%Width]/255][prev[y][(wx+1)%Width]/255][prev[(hy+1)%Height][(wx-1)%Width]/255][prev[(hy+1)%Height][x]/255][prev[(hy+1)%Height][(wx+1)%Width]/255];
        }
    //cout << duration_cast<milliseconds>(steady_clock::now() - start).count() << " ms\n";
}

template <typename T>
void update_texture(GLuint& tex, T world)
{
